...
struct CoreDataCatalogSelectionResolver {
    private let context: NSManagedObjectContext
    private let repository: CatalogSelectionRepository

    init(
        contextObject: AnyObject,
        repository: CatalogSelectionRepository = CatalogSelectionRepository()
    ) {
        self.context = (contextObject as? NSManagedObjectContext)
            ?? PersistenceController.shared.container.viewContext
        self.repository = repository
    }

    func addToCabinet(_ selection: CatalogSelection) throws -> SearchCatalogResolvedContext {
    ...
    func prepareTherapy(_ selection: CatalogSelection) throws -> SearchCatalogResolvedContext {
        let resolved = resolveOrCreateContext(for: selection, markInCabinet: true)
        resolved.entry.cabinet = nil
        try saveIfNeeded()
        return resolved
    ...
    func buyOnePackage(_ selection: CatalogSelection) throws -> SearchCatalogResolvedContext {
        let resolved = resolveOrCreateContext(for: selection, markInCabinet: true)
        resolved.entry.cabinet = nil
        try saveIfNeeded()
    ...
    func existingContext(for selection: CatalogSelection) -> SearchCatalogResolvedContext? {
        ...
        guard let entry = existingEntry(for: medicine, package: package) else { return nil }
        return SearchCatalogResolvedContext(
            selection: selection,
            medicine: medicine,
            package: package,
