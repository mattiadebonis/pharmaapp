...
struct ClinicalContext {
    ...
    let missedDoses: [TodayTodoItem]

    var allTodos: [TodayTodoItem] {
        monitoring + missedDoses
    }
}

struct ClinicalContextBuilder {
    ...
    func build(for medicines: [Medicine], now: Date = Date()) -> ClinicalContext {
        let therapies = medicines.flatMap { Array($0.therapies ?? []) }
    ...
    private func buildMonitoringTodos(
        events: [DoseEvent],
        therapiesByID: [NSManagedObjectID: Therapy],
        now: Date,
        startOfToday: Date,
        endOfToday: Date
    ) -> [TodayTodoItem] {
        ...
        for event in events {
            ...
            guard let rules = ClinicalRules.decode(from: therapy.clinicalRules) else { continue }
            ...
            for action in monitoring where action.schedule == nil {
                ...
                let id = "monitoring|dose|\(action.kind.rawValue)|\(relation.rawValue)|\(event.therapyId.uriRepresentation().absoluteString)|\(Int(event.date.timeIntervalSince1970))|\(Int(triggerDate.timeIntervalSince1970))"
                todos.append(
                    TodayTodoItem(
                        id: id,
    ...
    private func buildMissedDoseTodos(
        ...
        for event in events {
            ...
            if hasMatchingIntakeLog(for: event, therapy: therapy, tolerance: tolerance) {
                continue
            }

            let timeText = timeFormatter.string(from: event.date)
            let detail = "Dose delle \(timeText) non registrata"
            let id = "missed|\(event.therapyId.uriRepresentation().absoluteString)|\(Int(event.date.timeIntervalSince1970))"
            todos.append(
                TodayTodoItem(
                    id: id,
                    title: therapy.medicine.nome,
